...
Some of these methods are just simple convenience shortcuts (like ``get_energy()`` or ``get_final_molecule()``), others provide access to files in whatever formats a particular binary produces (XML, binary files).

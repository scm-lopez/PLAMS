...
For each such molecule an instance of single point job is created and stored in the ``children`` dictionary.
...
Any function that takes results of a single point job and return a single number can be used with the ``get_gradient`` method defined in  ``NumGradResults``
